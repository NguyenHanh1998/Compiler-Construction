...
void compileIfSt(void) {
  ...
  if (lookAhead->tokenType == KW_ELSE) {
    jInstrcution = genJ(DC_VALUE);
    updateFJ(fjInstruction, getCurrentCodeAddress());
    eat(KW_ELSE);
    compileStatement();
    updateJ(jInstrcution, getCurrentCodeAddress());
  }else {
    updateFJ(fjInstruction, getCurrentCodeAddress());
  } 
}
...
void compileForSt(void) {
  ...
  genDCT(1);//giam thanh ghi T1 don vi 
}
