  ...
  if(symtab->currentScope == NULL) 
    ...
    switch(obj->kind) {
      ...
      case OBJ_FUNCTION:
        ...
        obj->funcAttrs->scope->outer = scope;
        addObject(&(scope->objList), obj);
        break;
      case OBJ_PROCEDURE:
        scope = symtab->currentScope;
        obj->procAttrs->scope->outer = scope;
        addObject(&(scope->objList), obj);
        break;
      case OBJ_PARAMETER:
        scope = symtab->currentScope;
        obj->paramAttrs->localOffset = scope->frameSize;
        scope->frameSize += sizeOfType(obj->paramAttrs->type);
