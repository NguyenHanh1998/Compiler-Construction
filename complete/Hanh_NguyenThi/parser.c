...
//   if(type == SB_EQ || type == SB_NEQ || type == SB_ ) 
// }
...
void compileVarDecls(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      ...
      varType = compileType();
      for(int j = 0; j < i;j++) {
        varObj[j]->varAttrs->type = duplicateType(varType);
...
void compileAssignSt(void) {
  ...
  if(lookAhead->tokenType == SB_EQ || lookAhead->tokenType == SB_NEQ || lookAhead->tokenType == SB_LT || lookAhead->tokenType == SB_LE
