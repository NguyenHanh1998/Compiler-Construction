...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_IDENT:
    ...
    Object *identObject = lookupObject(currentToken->string);
    if(identObject == NULL) {
      error(ERR_UNDECLARED_IDENT, currentToken->lineNo, currentToken->colNo);
    }else if(identObject->kind != OBJ_CONSTANT) {
      error(ERR_INVALID_CONSTANT, currentToken->lineNo, currentToken->colNo);
    }

    constValue = duplicateConstantValue(identObject->constAttrs->value);
    break;
  case TK_CHAR:
    eat(TK_CHAR);
...
ConstantValue* compileConstant2(void) {
  ...
  case TK_IDENT:
    ...
    if(constObject == NULL){
      error(ERR_UNDECLARED_IDENT, currentToken->lineNo, currentToken->colNo);
    }else if(constObject->kind != OBJ_CONSTANT){
      error(ERR_INVALID_CONSTANT, currentToken->lineNo, currentToken->colNo);
    }
...
Type* compileType(void) {
  ...
  case KW_ARRAY:
    ...
    eat(SB_RSEL);
    eat(KW_OF);
    type = makeArrayType(sizeArray, compileType());
    break;
  case TK_IDENT:
    eat(TK_IDENT);
    Object *typeObject = lookupObject(currentToken->string);

    if(typeObject == NULL){
      error(ERR_UNDECLARED_IDENT, currentToken->lineNo, currentToken->colNo);
    }else if(typeObject->kind != OBJ_TYPE){
      error(ERR_INVALID_TYPE,currentToken->lineNo, currentToken->colNo);
    }
    type = duplicateType(typeObject->typeAttrs->actualType);
    break;
  default:
